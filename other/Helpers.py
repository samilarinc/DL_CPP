...
def gradient_check(layers, input_tensor, label_tensor):
    ...
    for i in range(input_tensor.shape[0]):
        for j in range(input_tensor.shape[1]):
            plus_epsilon = input_tensor.py_copy()
            plus_epsilon[i, j, 0] += epsilon
            minus_epsilon = input_tensor.py_copy()
            minus_epsilon[i, j, 0] -= epsilon

            activation_tensor = input_tensor.py_copy()
            ...
            analytical_derivative = error_tensor[i, j, 0]
            ...
            if normalizing_constant < 1e-15:
                ...
                difference[0, i, j] = np.abs(analytical_derivative - numerical_derivative) / normalizing_constant
    return difference
...
def gradient_check_weights(layers, input_tensor, label_tensor, bias):
    ...
    for it in np.ndindex(weights.shape[1:]):
        plus_epsilon = weights.py_copy()
        plus_epsilon[it] += epsilon
        minus_epsilon = weights.py_copy()
        minus_epsilon[it] -= epsilon
        ...
        analytical_derivative = analytical_derivative[it]

        if bias:
            layers[0].bias = plus_epsilon
        else:
            layers[0].weights = plus_epsilon
        plus_epsilon_activation = input_tensor.py_copy()
        for layer in layers[:-1]:
        ...
        minus_epsilon_activation = input_tensor.py_copy()
        for layer in layers[:-1]:
        ...
        if normalizing_constant < 1e-15:
            ...
            difference[it] = np.abs(analytical_derivative - numerical_derivative) / normalizing_constant
